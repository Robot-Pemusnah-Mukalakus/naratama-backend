...
router.get("/bookings", async (req, res) => {
  try {
    ...
    if (userId) where.userId = userId as Prisma.RoomBookingWhereInput["userId"];
    if (roomId) where.roomId = roomId as Prisma.RoomBookingWhereInput["roomId"];
    if (status) where.status = status as Prisma.RoomBookingWhereInput["status"];
    if (date) {
      const bookingDate = new Date(date as string);
      const startOfDay = new Date(bookingDate);
      startOfDay.setHours(0, 0, 0, 0);
...
router.put("/bookings/:id/status", async (req, res) => {
  try {
    const { paymentStatus, status } = req.body as Prisma.RoomBookingUpdateInput;
