  ...
  async (req, res) => {
    try {
      ...
      if (!hasActiveMembership) {
        ...
        // For now, return pending payment status
        return res.status(402).json({
          membershipRequired: false,
          message:
            "Payment required. Please complete the commitment fee and booking fee payment to proceed.",
          paymentDetails: {
            amount: USER_LIMITS.COMMITMENT_FEE_ROOM_BOOKING + totalCost,
