...
import { createTransactionBookLoan } from "#config/midtrans.js";
  ...
  async (req, res) => {
    try {
      ...
      if (!hasActiveMembership) {
        // Create pending loan first
        const pendingLoan = await prisma.bookLoan.create({
          data: {
            bookId: bookId as string,
        ...
        const transaction = await createTransactionBookLoan({
        ...
        return res.status(200).json({
          ...
          message: "Loan created. Please complete payment to confirm.",
          paymentDetails: {
            amount: USER_LIMITS.COMMITMENT_FEE_BOOK_LOAN,
            commitmentFee: USER_LIMITS.COMMITMENT_FEE_BOOK_LOAN,
            orderId: transaction.orderId,
          },
          success: true,
        });
